...
case class ListT[M[_], A](run: M[List[A]])(implicit M: Monad[M]) {
	...
	def filter(predicate: A => Boolean): ListT[M, A] = ListT(M.map(run)(_.filter(predicate)))
...
object ListT {
  ...
  implicit def richListTMonad[M[_], A](list: M[List[A]])(implicit M: Monad[M]) =  new {
    def toListT: ListT[M, A] =  ListT(list)
  }
}