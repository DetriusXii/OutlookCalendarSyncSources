...
class OutlookSyncAdapter(context: Context, autoInitialize: Boolean) extends AbstractThreadedSyncAdapter(context, autoInitialize) {
  ...
  def prepareQuery(context: Context): ListT[({type Q[A] = ValidationT[IO, IllegalArgumentException, A]})#Q, Calendar]  = {
    ...
    implicit def toList(calendarListValidationIO: ValidationT[IO, IllegalArgumentException, List[Calendar]])
    	= ListT.
    		richListTMonad[({type M[A] = ValidationT[IO, IllegalArgumentException, A]})#M, Calendar](
    		    calendarListValidationIO)(ValidationT.validationTMonad[IllegalArgumentException, IO])
    
    def helper = IO.using(new IO(() => context.getContentResolver().query(calendarURI, calendarProjection, null, null, null)))(cursor =>
    	grabCalendarRows(cursor).run.run 
    )
    
    (for (_ <- log("Writing to query the calendars: %s" format calendarURI.toString());
    		_ <- log("The calendar projection is %s" format calendarProjection.toString());
    		calendarRows <- helper
    ) yield calendarRows).toValidationT.toListT
  }
  ...
  private def handleCalendarInsertion(account: Account): OptionT[IO, Calendar] = {
    ...
    val properURI = CalendarContract.Calendars.CONTENT_URI.buildUpon().
    	    appendQueryParameter(CalendarContract.SyncColumns.ACCOUNT_NAME, account.name).
    ...
    val uriIO = insertions.insert(properURI, getContentValuesForCalendarInsert(account))
  ...
  override def onPerformSync(account: Account, 
    ...
    try {
      ...
      Log.v(OutlookSyncAdapter.TAG, "CalendarContractURI: %s" format CalendarContract.CONTENT_URI.toString())
      
      
      val password = accountManager.getPassword(account)
      log("Username: %s password: %s" format (account.name, password)).unsafePerformIO
      ...
      calendarsListT.length.getOrElse(illegalArgumentException => 
        log(illegalArgumentException.getMessage())
      ).flatMap(length => log("Length of list: %d" format length)).unsafePerformIO
      ...
      for (microsoftEvent <- readEventsFromURL(account);
    		  _ <- listTLiftIO[IO].liftIO(log("Microsoft Event: Start date: %s End date: %s" format (microsoftEvent.start.toString(), microsoftEvent.end)))
      ) yield ()
	  ...
	  def convertJSONTOMicrosoftEvent(jsonObject: JSONObject): Validation[ParseException, List[MicrosoftEvent]] = {
	    ...
	    val microsoftEventList = ((0 until values.length()) map (index => {
	      ...
	      val attendeesList =((0 until attendeesArray.length()) map (index => {
	        ...
	        val emailAddressObject = attendeesObject.getJSONObject("EmailAddress")
